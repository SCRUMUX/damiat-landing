...
import { getMockPotatoCrop } from './calculatorPriceApi';
import { useCalculatorCropData } from './useCalculatorCropData';
...
const INITIAL_VALUES: DamiatCalculatorFormValues = {
  hectares: '',
  yieldTonsPerHa: String(getMockPotatoCrop().defaultYieldTonsPerHa),
  device1: true,
...
export const DamiatCalculatorBlock: React.FC<DamiatCalculatorBlockProps> = ({
  ...
  const { crop: activeCrop } = useCalculatorCropData(priceApiUrl);
  ...
  }, [activeCrop, volumeTons, values.priceAdjustPercent, manualBase, values.salesTonsByMonth, values.opexRubByMonth]);

  const scheduleValidation: ScheduleValidation | null = result?.scheduleValidation ?? null;
